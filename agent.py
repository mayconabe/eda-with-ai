...
def get_analysis_code(user_prompt: str, sample_markdown: str) -> str:
    ...
    try:
        ...
        sys_analyst = (
            ...
            'Se fizer séries temporais, tente detectar colunas como ["date","data","dt","time","timestamp"] '
            '(case-insensitive) e converter com pd.to_datetime(errors="coerce").'
        )
        ...
        # Remoção defensiva de cercas, caso o modelo inclua por engano
        if code.startswith('```'):
            code = code.strip('`')
